...
* ステレオミキサーなど、肉声以外の入力を行うことができる手段を使用して、ボイスチャットチャンネルに次の入力を行う行為 (例外有り、下記参照)
  ...
  * 継続した不合理に大きい音量での入力
* Discordの表示名(アカウント名またはニックネーム)に装飾コードまたは「&」（アンパサンド）を含める行為
...
* [discord-spam-reporter](https://github.com/GiganticMinecraft/discord-spam-reporter)
  * 予め設定されたワードを含むメッセージが投稿された場合、本Botがそのメッセージを削除します。また、投稿を行ったユーザーを荒らしと判断し、当該ユーザーがどこのチャンネルにもメッセージをこれ以降投稿できないようにします。
  * 荒らしと判断された場合、運営チームがこれを確認し、適宜本手引きに基づいて処罰を別途行います。
  * 節度を持って利用していれば作動することはありません。
* [Vortex](https://github.com/jagrosh/Vortex)
  * 本Botにより、本手引き違反者に対しての期限BANや永久BANを実施しています。
...
　なお、荒らし対策のため、本グループに存在するボイスチャンネルをご利用いただくには専用のDiscordロールが必要です。詳しくは、グループにご参加の上#役職botチャンネルに記載されている内容をご覧ください。
...
　定期的に開催されるアイデア会議を行うときに利用するチャンネルです。アイデア会議については [[アイデア会議議事録:]] をご覧ください。
　聞き専の方も、#アイデア会議チャンネルを用いてテキストチャットによる会話への参加が可能です。
...
　オリジナル絵文字（スタンプ）が実装され、本グループ内の会話やリアクションに使用できる様になりました！
　上述のオリジナル絵文字を皆様からも募集します。だいたい正方形の画像を #絵文字募集 に投稿してもらえれば適当に採用します。
